...
export const tasks = sqliteTable('tasks', {
  ...
  type: text('type', { enum: ['美术', '测试', '文档', '配置', '资料整理', 'AI任务', '其他'] }).notNull().default('其他'),
  status: text('status', { enum: ['任务池', '今日待办', '进行中', '待验收', '需修改', '已完成', '阻塞'] }).notNull().default('今日待办'),
  priority: text('priority', { enum: ['低', '普通', '高', '紧急'] }).notNull().default('普通'),
  ...
  index('idx_tasks_assignee_planned_date').on(table.assignee, table.plannedDate),
...
export const workSessions = sqliteTable('work_sessions', {
  ...
  index('idx_work_sessions_assignee_started_at').on(table.assignee, table.startedAt),
]);
