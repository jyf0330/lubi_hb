...
大任务仅作粗略参考和状态汇总，不独立计时或计点。大任务预计时间允许为空，填写后也不要求等于小任务合计；小任务可以按实际执行粒度独立估时。小任务继续使用既有开始、暂停、继续、提交待验收流程。大任务只有所有子任务验收通过才显示已完成。混合已完成/待验收显示待验收，返工和阻塞优先反映。负责人看板增加汇总和所属大任务名称，原有每日统计只统计子任务，不重复计数。负责人“优先关注”按大任务一级汇总，展开二级界面后查看并处理其中的小任务；没有大任务归属的任务继续直接显示。

大任务整体验收（2026-09-20 起）：全部小任务都提交待验收后，“优先关注”只为该大任务保留一条汇总项，直接显示员工建议总分（各小任务自评分合计）并提供“一次验收大任务”按钮；点击即按每个小任务自己的自评分一次性通过、计分并归属审核通过日。未全部提交前，小任务的“待验收”不单独进入该队列（“需修改”和“阻塞”仍分别显示）；任一小任务缺少自评分时按钮禁用并提示先补齐；重复一次验收被拒绝。
