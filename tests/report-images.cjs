...
(async()=>{
  ...
  const taskHtml=helpers.taskAttachmentGallery([{id:'task-image',name:'截图.webp',content_type:'image/webp',size:1024},{id:'task-file',name:'交付.zip',content_type:'application/zip',size:1024}],new URL('https://example.test/team-board/api/'));
  assert.match(taskHtml,/https:\/\/example\.test\/team-board\/api\/task-files\/task-file/);
  assert.match(taskHtml,/https:\/\/example\.test\/team-board\/api\/task-files\/task-image/);
